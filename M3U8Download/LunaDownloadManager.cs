...
internal class LunaDownloadManager
{
    ...
    private void ChangeSpecInfo(StreamSpec streamSpec, List<Mediainfo> mediainfos, ref bool useAACFilter)
    {
        ...
        if (mediainfos.Where(m => m.Type == "Audio").All(m => m.BaseInfo!.Contains("aac"))) useAACFilter = true;
        ...
        SpeedContainer speedContainer)
    {
        ...
        {
            var splitSegments =
                await LargeSingleFileSplitUtil.SplitUrlAsync(segments.First(), DownloaderConfig.Headers);
        ...
        if (!readInfo || StreamExtractor.ExtractorType == ExtractorType.MSS)
        {
            ...
            if (result is { Success: true })
            {
                ...
                // 读取init信息
                if (string.IsNullOrEmpty(currentKID)) currentKID = MP4DecryptUtil.GetMP4Info(result.ActualFilePath).KID;
                // try shaka packager, which can handle WebM
                if (string.IsNullOrEmpty(currentKID) &&
                    DownloaderConfig.MyOptions.DecryptionEngine == DecryptEngine.SHAKA_PACKAGER)
                {
            ...
            mp4InitFile != "")
        {
            ...
            if (decryptEngine != DecryptEngine.MP4DECRYPT) FileDic!.Remove(streamSpec.Playlist!.MediaInit, out _);
        }
        ...
        if (DownloaderConfig.MyOptions.AutoSubtitleFix &&
            ...
            streamSpec.Extension.Contains("vtt"))
        {
